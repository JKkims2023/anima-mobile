/**
 * ═══════════════════════════════════════════════════════════════════════════
 * 🎯 QuickActionChipsSage Component (SAGE Mode)
 * ═══════════════════════════════════════════════════════════════════════════
 * 
 * Quick action chips for SAGE mode
 * - 설정 (Settings)
 * - 알림 (Notifications) - with NEW badge
 * 
 * Features:
...
const QuickActionChipsSage = ({
  onSettingsClick,
  onNotificationClick,
  badgeData = {}, // { notificationHasNew }
}) => {
  const insets = useSafeAreaInsets();
  const { showTooltips } = useQuickAction();
  const [visibleTooltips, setVisibleTooltips] = useState({});
  
  // ✅ Actions configuration (SAGE only has 2 actions)
  const actions = [
    {
      id: 'settings',
      icon: 'cog',
    ...
    },
    {
      ...
      badge: badgeData.notificationHasNew ? { type: 'NEW' } : null,
    },
  ];
  
  // ✅ Animation values (fixed count: 2 chips for SAGE)
  ...
  useEffect(() => {
    if (__DEV__) {
      console.log('[QuickActionChipsSage] 🎬 Starting entry animation');
    }
    ...
    return () => {
      if (__DEV__) {
        console.log('[QuickActionChipsSage] 🧹 Cleaning up animations');
      }
        ...
        return (
          <View key={action.id} style={styles.chipWrapper}>
            ...
            >
              ...
              {/* Badge */}
              {action.badge && (
                <QuickActionBadge type={action.badge.type} />
              )}
            </AnimatedTouchable>
...
const styles = StyleSheet.create({
  ...
  tooltip: {
    position: 'absolute',
    left: scale(-100), // Position to the left of chip
    top: '50%',
...
export default QuickActionChipsSage;
